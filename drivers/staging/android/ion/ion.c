...
#if TSAI
	buffer->tsai_seqno = ++tsai_ion_seqno; /* defined in ion_priv.h */
#if TSAI_DS5
	SRUK_ANNOTATE_CHANNEL_COLOR(2, ANNOTATE_YELLOW, "ion_buffer_create %d", buffer->tsai_seqno);
#endif
#endif
	buffer->heap = heap;
	buffer->flags = flags;
...
#if TSAI_DS5
	SRUK_ANNOTATE_CHANNEL_COLOR_TS(3, ANNOTATE_GREEN, &ts, "ion_share_dma_buf_fd %d buf %d", fd, handle->buffer->tsai_seqno);
...
unsigned int ion_handle_get_tsai_seqno(struct ion_handle *handle) {
	return handle->buffer->tsai_seqno;
}
